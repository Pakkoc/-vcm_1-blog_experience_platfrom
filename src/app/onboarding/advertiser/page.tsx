    ...
    const restoreSession = async () => {
      ...
      try {
        ...
        if (restored) {
          ...
          console.warn(
            "[Advertiser Onboarding] 인증 파라미터가 없어 세션 복원이 필요하지만 토큰을 찾지 못했습니다."
          );
        }
      } catch (error) {
        console.error(
          "[Advertiser Onboarding] Unexpected session restore error",
          error
        );
      } finally {
        setIsRestoringSession(false);
  ...
  return (
    <div className="container mx-auto min-h-screen max-w-4xl px-6 py-16">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold">광고주 정보 등록</h1>
